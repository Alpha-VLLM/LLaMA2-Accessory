...
from data.conversation.dataset import DialogProcessor
from data.data_reader import extract_frame_from_video
...
def model_worker(
    ...
    dialog_processor = DialogProcessor(
        None, args.model_max_seq_len, model.get_media_words(), model.tokenizer
    )
    ...
    while True:
        ...
        try:
            image, video, chatbot, max_gen_len, temperature, top_p, img_transform = request_queue.get()
            dialog_processor.transform = get_transform(img_transform, getattr(model.llma, 'image_size', 224))
            # video
            item = {"conversations": []}
            for q, a in chatbot:
                item['conversations'].append({"from": "human", "value": q})
                item['conversations'].append({"from": "gpt", "value": a})
            if video is not None:
                frames = extract_frame_from_video(video, 10)
                item["video_frame"] = frames

            tokenized_prompt, _, additional = dialog_processor.process_item(data_item=item)
            for key in additional:
                additional[key]['data'] = [_.cuda() for _ in additional[key]['data']]
            additional = {key: [val] for key, val in additional.items()}

            with torch.autocast(dtype=target_dtype, device_type="cuda"):
                print(tokenized_prompt.tolist())
                for stream_response in model.stream_generate(
                    tokenized_prompt[None], additional,
...
def gradio_worker(
    ...
    def show_user_input(msg, chatbot, chatbox_display):
        return "", chatbot + [[msg, None]], chatbox_display + [[msg.replace("<", "&lt;").replace(">", "&gt;"), None]]

    def stream_model_output(img, video:str, chatbot, chatbot_display, max_gen_len, gen_t, top_p, img_transform):
        while True:
            content_piece = response_queue.get()
            if isinstance(content_piece, Ready):
                break
        for queue in request_queues:
            queue.put((img, video, chatbot, max_gen_len, gen_t, top_p, img_transform))
        while True:
            content_piece = response_queue.get()
    ...
    with gr.Blocks(css="#image_input {height: 100% !important}") as demo:
        ...
        with gr.Row() as r:
            ...
            with gr.Column(scale=1):
                video_input = gr.Video(label='video Input', format='mp4', elem_id="image_input")
        ...
        msg.submit(
            ...
            stream_model_output, [img_input, video_input, chatbot, chatbot_display, max_gen_len, gen_t, top_p, img_transform],
