...
from data.llama2_chat import B_INST, E_INST, B_SYS, E_SYS, DEFAULT_SYSTEM_PROMPT
...
def model_worker(
    ...
    while True:
        ...
        conv = [{"role": "system", "content": DEFAULT_SYSTEM_PROMPT}]
        for user, bot in chatbot:
            conv.append({"role": "user", "content": user})
            if bot is not None:
                conv.append({"role": "assistant", "content": bot})
        conv = [
                {
                    "role": conv[1]["role"],
                    "content": B_SYS
        ...
        conv_tokens = sum(
            [
                tokenizer.encode(
                    ...
                    bos=True, eos=True)
                for prompt, answer in zip(conv[::2], conv[1::2])
            ],
            [],
        )
        assert (
            conv[-1]["role"] == "user"
        ), f"Last message must be from user, got {conv[-1]['role']}"
        conv_tokens +=tokenizer.encode(
            f"{B_INST} {(conv[-1]['content']).strip()} {E_INST}",
            bos=True,
            eos=False,
        )
